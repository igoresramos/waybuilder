...
- `avatar/fontes/` -- clone do LPC no pin. **Fora do git**, reconstruivel por
  `avatar/buscar_fonte.sh` (mesmo criterio de `pipeline/dados_brutos/`).
- `avatar/saida/` -- atlas, catalogo e creditos. **Versionado**, porque e o que
  o deploy consome e nao ha como regera-lo la.
...
**10. Nao existe app separado.** O prototipo e uma **rota de dev dentro do
proprio repo**: mesmo build, mesma pipeline de assets, mesmo versionamento. A
promocao a modal e mover um componente.

> Prototipo em projeto separado e modo de perda conhecido nesta casa -- foi
> assim que o HTML da home do nimbulus-web morreu num `/tmp`.
