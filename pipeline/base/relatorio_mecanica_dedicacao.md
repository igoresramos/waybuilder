# Mecanica de dedicacao derivada da prosa

- dedicacoes sem `grants` na entrada: **60**
- mecanizadas aqui: **5**
- divida declarada (modelo que o motor nao tem): **25**
- sem padrao reconhecido: **30**

Cada linha traz a FRASE que justifica a concessao. Divergencia entre a frase e o `grants` e defeito deste passo, nao do dado.

## Derivadas

| dedicacao | concede | frase na prosa | nao derivado |
|---|---|---|---|
| Butterfly Blade Dedication | `{"skill_training": {"auto": ["deception"]}}; {"skill_training": {"auto": ["stealth"]}}` | trained in deception; trained in stealth | - |
| Prophet of Kalistrade Dedication | `{"proficiency": {"society": "expert"}}` | expert in society | - |
| Rose Warden Dedication | `{"skill_training": {"auto": ["deception"]}}` | trained in deception | escolha: Deception, you become an expert in your choice of  |
| Undead Master Dedication | `{"grant_feat": ["wb:action/call-companion"]}` | gain the Call Companion action | - |
| Vehicle Mechanic Dedication | `{"proficiency": {"crafting": "expert"}}` | expert in crafting | master in crafting (rank posterior) |

## Divida declarada -- exige modelo novo no motor

| dedicacao | o que a prosa concede |
|---|---|
| Animal Trainer Dedication | companheiro |
| Captivator Dedication | escolha: that skill, or become an expert if you were alread |
| Cavalier Dedication | companheiro |
| Clockwork Reanimator Dedication | companheiro |
| Curse Maelstrom Dedication | companheiro |
| Drake Rider Dedication | companheiro |
| Elementalist Dedication | companheiro |
| Geomancer Dedication | spellcasting |
| Golden League Xun Dedication | escolha: two of the following skills (or in two other skill |
| Halcyon Speaker Dedication | spellcasting |
| Hallowed Necromancer Dedication | spellcasting |
| Iridian Choirmaster Dedication | companheiro |
| Living Monolith Dedication | escolha: Ancient Osirion Lore, or expert if you were alread |
| Magaambyan Attendant Dedication | companheiro |
| Mammoth Lord Dedication | companheiro |
| Oatia Skysage Dedication | spellcasting |
| Reanimator Dedication | spellcasting |
| Scion of Domora Dedication | companheiro |
| Sixth Pillar Dedication | escolha: Athletics, or in another trained skill if you were |
| Sniping Duo Dedication | companheiro |
| Soul Warden Dedication | companheiro |
| Spell Trickster Dedication | companheiro |
| Turpin Rowe Lumberjack Dedication | escolha: Milling Lore and Forest Lore, or an expert if you , companheiro |
| Vigilante Dedication | companheiro |
| Wellspring Mage Dedication | spellcasting |

## Sem padrao reconhecido -- revisar a mao

- Animist Dedication
- Artillerist Dedication
- Blessed One Dedication
- Firebrand Braggart Dedication
- Flexible Spellcaster Dedication
- Ghost Eater Dedication
- Ghost Hunter Dedication
- Gladiator Dedication
- Martial Artist Dedication
- Mind Smith Dedication
- Nantambu Chime-Ringer Dedication
- Nidalese Horselord Dedication
- Overwatch Dedication
- Runelord Dedication
- Sanguimancer Dedication
- Scout Dedication
- Scrollmaster Dedication
- Scrounger Dedication
- Shadowcaster Dedication
- Soulforger Dedication
- Staff Acrobat Dedication
- Starstone Aspirant Dedication
- Thlipit Contestant Dedication
- Time Mage Dedication
- Trapsmith Dedication
- Trick Driver Dedication
- Venture-Gossip Dedication
- Viking Dedication
- Weapon Improviser Dedication
- Wild Mimic Dedication
