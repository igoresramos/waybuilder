...
class TestDedicacaoPrimeiro(unittest.TestCase):
    ...
    def test_lacuna_feat_de_arquetipo_sem_has_no_requires(self):
        """LACUNA. Barbarian Resiliency exige Barbarian Dedication no RAW, mas
        o `requires` da base so traz `character_level >= 4`. Sao 181 feats de
        arquetipo nessa situacao (medido no pin atual). O motor le so o
        `requires`, entao nao ha sinal nenhum -- e o dado para deduzir existe:
        `feat["archetype"]` aponta o arquetipo e a dedicacao dele tem trait
        `dedication`."""
...
class TestSlotSoAceitaFeatDeArquetipo(unittest.TestCase):
    ...
    @unittest.expectedFailure
    def test_lacuna_class_feat_no_slot_de_arquetipo(self):
        """LACUNA. Reactive Shield nao tem trait `archetype` e mesmo assim
        ocupa um slot de Free Archetype sem gerar sinal. O motor nunca compara
        o tipo do feat com o slot que o recebeu."""
        p = carregar("guerreiro4-fa-class-feat-no-slot.json")
        self.assertTrue(sinal_de_arquetipo(p), "nada sinalizado")
...
class TestNovaDedicacaoExigeDoisFeats(unittest.TestCase):
    ...
    @unittest.expectedFailure
    def test_lacuna_segunda_dedicacao_sem_dois_feats(self):
        """LACUNA. A regra nao existe no motor: nem `requires` da base nem
        codigo. Marshal Dedication entra no nivel 4 com zero feats de Archer
        acumulados e nada e apontado."""
        p = carregar("guerreiro6-fa-duas-dedicacoes.json")
...
class TestHigieneDoSlot(unittest.TestCase):
    ...
    @unittest.expectedFailure
    def test_lacuna_escolha_em_nivel_impar(self):
        """LACUNA. Um pick de free_archetype no nivel 3 nao gera aviso -- o
        motor coleta `gastos` sem confrontar com `slots`."""
        def para_impar(d):
            for e in d["escolhas"]:
    ...
    @unittest.expectedFailure
    def test_lacuna_mais_escolhas_que_slots(self):
        """LACUNA. Tres feats de arquetipo num personagem com dois slots
        passam sem aviso."""
